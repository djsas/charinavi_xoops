...
if(isset($_POST["id"]) && isset($_POST["type"]) && in_array($_POST["type"], array("change", "delete"))){
	...
	if($_POST["type"] == "change" && is_uploaded_file($_FILES["imgfile_".$id]["tmp_name"])){
		...
		$sql = sprintf("UPDATE %s SET image = '%s', imagetype = '%s' WHERE id = %s;", $xoopsDB->prefix("charinavi_category"), $img, $imgtype, $id);
		$xoopsDB->query($sql);
...
while($row = $xoopsDB->fetchArray($res)){
	...
	if($row["image"]){
		$img = XOOPS_URL."/modules/charinavi/resizeimg.php?id=".$id;
		//print $img;
	}else{
		$img = XOOPS_URL."/modules/charinavi/images/noimage.jpg";
	}
	$html .= sprintf('<tr><td>%s</td><td><img src="%s" style="width:70px; height:70px;" /><input type="file" name="imgfile_%s" /><input type="button" name="changeimg" value="%s" onclick="changeImage(%s, \'change\');" /></td><td><input type="button" name="delete" value="%s" onclick="changeImage(%s, \'delete\');" /></td></tr>',
